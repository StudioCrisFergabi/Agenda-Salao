...
function boot() {
  ...
  // O listener de login precisa ser registrado imediatamente — não pode
  // depender do Firestore estar pronto (se as regras do Firestore ainda não
  // estiverem publicadas, por exemplo, isso travava o login silenciosamente:
  // a pessoa conseguia autenticar, mas o painel nunca aparecia).
...
async function doLogin() {
  ...
  try {
    ...
    await Promise.race([signInWithEmailAndPassword(window.FIREBASE_AUTH, email, pass), timeout]);
    loginError.textContent = '';
  } catch (err) {
    console.error(err);
    const code = err.code || '';
    if (err.message === 'TIMEOUT') {
      loginError.textContent = 'A conexão está demorando demais. Verifique sua internet e tente de novo.';
...
async function initDashboard() {
  ...
  await renderAgenda().catch(err => console.error(err));
